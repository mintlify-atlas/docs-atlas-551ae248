...
| NOT_SYNCED_YET | The node is still syncing and the requested block is not in the database yet | Wait until the node finish syncing<br/>Send a request to a different node which is synced |
