...
Gas in NEAR is computed on **gas units** and charged using $NEAR tokens based on the network's **gas price**.
...
<Note>
  The fee is in $NEAR, to obtain the cost in dollars multiply by the current price of $NEAR
</Note>

<Accordion title="Function Calls*">
  The cost of calling a function will depend on how complex the function is, but will be consistent across function calls.
</Accordion>

<Accordion title="Deploying a Contract**">
...
<Accordion title="Where do these numbers come from?">
  ...
  When you make a request to transfer funds, NEAR immediately deducts the appropriate `send` amount from your account. Then it creates a receipt, an internal book-keeping mechanism. Creating a receipt has its own associated costs:
...
You don't buy gas, instead, the gas fee is automatically removed from your account's balance when the transaction is first processed based on the action's gas cost and the network's gas price.
...
<Accordion title="Gas Refund Fee">
  ...
  **But why introducing such a fee instead of refunding all gas?**
