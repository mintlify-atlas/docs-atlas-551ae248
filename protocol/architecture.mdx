...
NEAR consists roughly of a blockchain layer and a runtime layer. These layers are designed to be independent from each other: the blockchain layer can in theory support runtime that processes transactions differently, has a different virtual machine (e.g. RISC-V), has different fees; on the other hand the runtime is oblivious to where the transactions are coming from. It is not aware whether the blockchain it runs on is sharded, what consensus it uses, and whether it runs as part of a blockchain at all.
...
Similar to Ethereum, Near Protocol is an [account-based system](/protocol/account-model). Which means that each blockchain user is roughly associated with one or several accounts (there are exceptions though, when users share an account and are separated through the [access keys](/protocol/access-keys)).
...
Blockchain layer however is mostly aware of the accounts through [the trie](#trie) and [the validators](#validators). Outside these two it does not operate on the accounts directly.
...
Even though tokens is a fundamental concept of the blockchain, it is neatly encapsulated inside the runtime layer together with the [gas](/protocol/gas), fees, and rewards.
...
Both the blockchain layer and the runtime layer are aware of a special group of participants who are responsible for maintaining the integrity of the Near Protocol. These participants called [Validators](/protocol/validators) are associated with the accounts and are rewarded accordingly. The reward part is what the runtime layer is aware of, while everything around the orchestration of the validators is inside the blockchain layer.
...
<Tabs>
  <Tab title="Sharding">
    The runtime layer does not know that it is being used in a sharded blockchain, e.g. it does not know that the trie it works on is only a part of the overall blockchain state
  </Tab>
  <Tab title="Blocks or chunks">
    The runtime does not know that the receipts that it processes constitute a chunk and that the output receipts will be used in other chunks. From the runtime perspective it consumes and outputs batches of transactions and receipts
  </Tab>
  <Tab title="Consensus">
    The runtime does not know how consistency of the state is maintained
  </Tab>
  <Tab title="Communication">
    The runtime does not know anything about the current network topology. Receipt has only a `receiver_id` (a recipient account), but knows nothing about the destination shard, so it's a responsibility of the blockchain layer to route a particular receipt
  </Tab>
</Tabs>
...
<Card title="Fees and rewards" icon="coins">
  Fees and rewards are neatly encapsulated in the runtime layer. The blockchain layer, however, has an indirect knowledge of them through the computation of the tokens-to-gas exchange rate and the inflation.
</Card>

<Info>
  The clear separation between blockchain and runtime layers allows NEAR to be flexible and adaptable while maintaining a clean architecture.
</Info>